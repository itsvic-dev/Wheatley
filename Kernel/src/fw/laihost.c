...
__attribute__((noreturn)) void laihost_panic(const char *msg) { panic(msg, 0); }
...
void laihost_pci_writeb(uint16_t seg, uint8_t bus, uint8_t slot, uint8_t fun,
  ...
  pci_device_path_t device = {seg, bus, slot, fun};
...
// sleep for N ms
void laihost_sleep(uint64_t ms) { mdelay(ms); }

uint64_t laihost_timer(void) { return hpet_time(); }
